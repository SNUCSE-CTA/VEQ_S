...
public:
  ...
  Graph()
  {
    ...
    vertexOffset = NULL; // vertexOffset[l]: the offset of the starting position of vertices with label l
    maxNbrDegree = NULL;
    #ifdef NEIGHBOR_LABEL_FREQUENCY
    #else
    NLF = NULL;
  ...
  inline void computeNLF()
  {
	...
	if(cntLabel[lid] < BITS_PER_LABEL)
	{
          ...
          #ifdef HUGE_GRAPH
	  NLF[(long long)j  * (long long)NLFSize + (long long)idx] |= (1ULL << pos);
          #else
  ...
  void print(int id=0)
  {
    unordered_map<pair<int, int>, pair<int, int>, pairHash>::const_iterator iter;
    cout<<"[Graph "<<id<<"]"<<endl;
    cout<<"|V|: "<<nVertex<<". |E|: "<<nEdge<<". |L|: "<<nLabel<<". maxD: "<<maxDegree<<endl;
    for(int i = 0; i < nVertex; ++i){
      cout<<"(u"<<i<<") lv: "<<label[i]<<". degree: "<<degree[i]<<". nbr:";
	...
	cout<<"\tN(u: u"<<i<<", lv: "<<j<<"):";
	for(int k = iter->second.first; k < iter->second.first + iter->second.second; ++k){
	  cout<<" u"<<nbr[k];
	}
	cout<<endl;
      }
